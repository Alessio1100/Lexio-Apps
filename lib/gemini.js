...
function buildPrompt({ transactions, categories, examples, rulesText }) {
  ...
  lines.push(
    ...
    ...transactions.map(
      (t) =>
        `- id=${t.id} | esercente="${(t.merchant || "").slice(0, 60)}" | importo=${t.amount} | valuta=${t.currency || "EUR"} | estero=${t.foreign ? "si" : "no"} | banca="${t.bank || ""}" | descrizione="${(t.description || "").slice(0, 120)}"`
    )
  );
  return lines.join("\n");
}
