...
export function normalizeTransaction(raw, { connectionId, userId, currency }) {
  ...
  // Controparte = l'ALTRA parte rispetto al titolare del conto:
  // per un'uscita è il beneficiario (creditor), per un'entrata è il mittente (debtor).
  const isOutgoing = signed < 0;
  const merchant =
    (isOutgoing ? raw.creditor?.name : raw.debtor?.name) ||
    raw.creditor?.name ||
    raw.debtor?.name ||
