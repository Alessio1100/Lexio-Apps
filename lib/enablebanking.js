...
async function ebFetch(path, { method = "GET", body, headers } = {}) {
...
// psu = { ip, userAgent }: se presente, invia gli header PSU (richiesta "presidiata",
// cliente presente) → l'ASPSP non applica il limite giornaliero degli accessi non
// presidiati (PSD2). Usato per il sync manuale. Nota: gli header PSU vanno tutti o
// nessuno, altrimenti l'API risponde PSU_HEADER_NOT_PROVIDED.
export async function getAccountTransactions(uid, { dateFrom, continuationKey, psu } = {}) {
  const qs = new URLSearchParams();
  if (dateFrom) qs.set("date_from", dateFrom);
  if (continuationKey) qs.set("continuation_key", continuationKey);
  const s = qs.toString();
  const headers =
    psu && psu.ip && psu.userAgent
      ? { "Psu-Ip-Address": psu.ip, "Psu-User-Agent": psu.userAgent }
      : undefined;
  return ebFetch(`/accounts/${uid}/transactions${s ? `?${s}` : ""}`, { headers });
}
...
export function counterpartyFromRaw(raw) {
  if (!raw) return ""; // spese manuali/contanti: nessun dato grezzo
...
// Bonifico BuddyBank in uscita: la controparte non arriva strutturata, tutto è
// nella descrizione grezza, es.:
//   "DISPOSIZIONE DI BONIFICO BONIFICO ISTANTANEO DEL 10.08.2026 ALLE 17.28.33
//    A  SILVIA GIALLORETO PER  Rimborso eccedenza viaggio TRN  119... COMM 0 ..."
// Estrae { recipient, causale } (causale può essere vuota). Ritorna null se non
// è un bonifico in uscita in questo formato.
export function outgoingTransferParts(description) {
  const d = (description || "").replace(/\s+/g, " ").trim();
  const m = /DISPOSIZIONE DI BONIFICO.*?\bA\s+(.+?)\s+PER\s+(.*?)\s*(?:\bTRN\b|\bCOMM\b|$)/i.exec(d);
  if (!m) return null;
  const recipient = cleanMerchant(m[1]);
  if (!recipient) return null;
  const causale = (m[2] || "").replace(/\s+/g, " ").trim();
  return { recipient, causale };
}
...
export function deriveMerchant(raw, description) {
  ...
  if (m) return cleanMerchant(m[1]);
  // bonifico in uscita (formato BuddyBank "DISPOSIZIONE DI BONIFICO … A <dest> PER …")
  const tp = outgoingTransferParts(d);
  if (tp) return tp.recipient;
  // bonifico in uscita (altri formati "A FAVORE DI …")
  m = d.match(/A FAVORE DI\s+(.+?)(?:\s+PER\b|\s+TRN\b|$)/i);
  if (m) return cleanMerchant(m[1]);
  return cleanMerchant(d);
}

// Nome per la UI (card lista spese): come deriveMerchant ma, per i bonifici in
// uscita, aggiunge la causale se presente → "Destinatario · causale".
// La causale NON entra in merchant_name (che resta la controparte stabile, usata
// per raggruppare spese fisse e memoria per esercente).
export function deriveDisplayName(raw, description) {
  if (!counterpartyFromRaw(raw)) {
    const tp = outgoingTransferParts(description);
    if (tp) return tp.causale ? `${tp.recipient} · ${tp.causale}` : tp.recipient;
  }
  return deriveMerchant(raw, description);
}
